...
3. **Resolve merge conflicts immediately.** Keep `feature/task-7-implementation` in a mergeable state at all times.
...
5. **Label discipline.** Ensure the PR carries `task-7`, `service-rust-basic-api`, and `run-play-workflow-template-fzhj8`. Apply or remove `ready-for-qa` based on CI status.
...
- PR must exist with the proper branch (`feature/task-7-implementation`) and labels.
