...
5. **Stay on the feature branch.** The controller has already checked out `feature/task-1-implementation` for you. Double-check `git status` before committing, and never push directly to `main`/`master`.
6. **Task isolation is absolute.** If you discover gaps outside Task 1, leave a note but do not implement them.
...
7. **Create the PR**: `gh pr create ...` with task-specific title/body, add labels (`task-1`, `service-rust-basic-api`, `run-play-workflow-template-tzf9r`), and capture test results (reuse the narrative above in the PR body).
