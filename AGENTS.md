...
3. **Resolve merge conflicts immediately.** Keep `feature/task-6-implementation` in a mergeable state at all times.
...
5. **Label discipline.** Ensure the PR carries `task-6`, `service-rust-basic-api`, and `run-play-workflow-template-zqlcw`. Apply or remove `ready-for-qa` based on CI status.
...
- PR must exist with the proper branch (`feature/task-6-implementation`) and labels.
