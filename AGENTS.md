...
5. **Task isolation is absolute.** If you discover gaps outside Task 1, leave a note but do not implement them.
...
6. **Create the PR**: `gh pr create ...` with task-specific title/body, add labels (`task-1`, `service-rust-basic-api`, `run-play-workflow-template-8nlbk`), and capture test results.
